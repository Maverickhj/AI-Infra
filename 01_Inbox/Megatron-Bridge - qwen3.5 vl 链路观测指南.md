...
aliases: []
source:
  - https://github.com/Maverickhj/Megatron-Bridge/blob/codex/dev-v0.5.1-minimind/docs/debug/qwen3.5%20vl%20%E9%93%BE%E8%B7%AF%E8%A7%82%E6%B5%8B%E6%8C%87%E5%8D%97.md
sync_source: docs/debug/qwen3.5 vl 链路观测指南.md
ai_generated: false
reviewed: false
---
...
> 来源：https://github.com/Maverickhj/Megatron-Bridge/blob/codex/dev-v0.5.1-minimind/docs/debug/qwen3.5%20vl%20%E9%93%BE%E8%B7%AF%E8%A7%82%E6%B5%8B%E6%8C%87%E5%8D%97.md
...
> [!note] 文档定位
> 本文是 [[megatron-lm 链路可观测性 roadmap]] 的 **Qwen3.5-VL SFT 测试实例**，
> 负责回答“这个具体模型场景下应观察什么、预期看到什么”。通用观测原则、L0-L5 分层
> 定义和实施 roadmap 统一放在通用文档中，不在本文重复维护。
...
本文基于通用观测 roadmap，把以下信息在 Qwen3.5-VL SFT 上关联起来：
...
观测能力与 L0-L5 分级、工具选择和实施顺序见 [[megatron-lm 链路可观测性 roadmap]]。
本文只补充 Qwen3.5-VL SFT 特有的调用链、配置影响、最小实验集和验收口径。
...
- [[#8. 测试场景的观测落点|8. 测试场景观测落点]]
- [[#9. 配置对关键路径的影响|9. 配置影响]]
- [[#10. 本测试场景的执行顺序|10. 执行顺序]]
...
通用 L0-L5 分层、工具选择和实施 roadmap 见
[[megatron-lm 链路可观测性 roadmap]]。本节只记录 Qwen3.5-VL SFT 场景下的关注点和预期证据。

| 层级 | 本测试场景的落点 | 关键预期 |
| --- | --- | --- |
| L0 | 在 `setup()` 完成后 dump `ProcessGroupCollection`、schedule 名称、wrapper/optimizer 类型 | 单卡 PP=1 时为 `forward_backward_no_pipelining`；模型是 `Qwen3VLModel`，包含 vision + language model |
| L1 | 打开 MCore timers，抓稳态 iteration 的阶段耗时 | `forward-backward`、`optimizer-*` 各阶段可对齐；GDN/attention 层差异不在 timer 中体现 |
| L2 | 当前延后，必要时只选少量 module 做 shape probe | 优先用 L3；待解释 backward/recompute 时再启用 |
| L3 | PyTorch profiler 开 shape/callstack，抓 1-2 个稳态 iteration | 能看到 `Qwen3VLModel.forward()` 下游的 ATen/TE op 与输入输出 shape |
| L4 | Nsys + NVTX，抓同一个稳态 step | 确认单卡下通信是否真的不产生 NCCL kernel，以及 vision/language/optimizer 在 stream 上的先后 |
| L5 | 按 11 节实验 B/C/D 做单变量对照 | TP=2+SP、PP=2 分别引入不同通信和不同 layer range |

Qwen3.5-VL 的 L4 单卡启动示例保留在通用 roadmap 的基础上，只需替换 training script：
...
通用实施 roadmap 见 [[megatron-lm 链路可观测性 roadmap]]。在本测试场景中按以下顺序推进：

1. 先跑通 Qwen3.5-VL SFT 单卡基线，确认 `setup()` 后能拿到 `ProcessGroupCollection` 和 schedule；
   最快入口可先用 [[qwen35-vl-800m-sft-smoke]] 的 1-step mock smoke，避免真实数据和 checkpoint。
2. 完成 L0 topology JSON，并跑通 L1 timer 与 L3 PyTorch profiler shape 采集；
3. 用 11.1 实验 A 把 timer、profiler timeline 和预期 phase 对齐；
4. 再依次跑 11.2-11.4 的单变量对照，验证 L5 通信变化；
5. 只有 L3/L4 无法解释 backward/recompute 重复调用时，才回到 roadmap Phase 2 引入 module JSONL。
...
- 开 L0 topology、L1 timer 和 L3 PyTorch profiler shape；
